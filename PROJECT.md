...
這個選型保留純靜態部署，同時讓經歷資料與頁面結構分離。首頁仍保留自我介紹、核心能力、工作經歷與經歷總覽精選項目的靜態 fallback，避免重要資訊只存在 JavaScript 渲染結果中。
...
為了避免資料只存在 JavaScript 渲染結果中，重要摘要仍同步維護於 `index.html`、`llms.txt` 與 JSON-LD。`index.html` 內保留可直接讀取的自我介紹、核心能力、工作經歷與經歷總覽精選項目。
