...
`assets/js/site.js` 負責讀取 `data/experiences.json`，並渲染工作經歷、代表專案案例與經歷總覽。經歷總覽會依分類顯示學歷、獎項證照與榮譽、比賽經歷、課程與訓練、專案、演講、社群與研討會參與；每個分類預設顯示精選或前三筆資料，使用者可按「展開更多某分類」查看該分類完整清單。

這個選型保留純靜態部署，同時讓經歷資料與頁面結構分離。首頁仍保留現職、工作經歷、代表重點、代表專案與經歷總覽精選項目的靜態 fallback，避免重要資訊只存在 JavaScript 渲染結果中。
...
| `/assets/js/site.js` | 經歷資料、代表專案案例渲染與展開互動 |
| `/data/experiences.json` | 前端讀取的經歷資料與代表專案案例資料 |
...
為了避免資料只存在 JavaScript 渲染結果中，重要摘要仍同步維護於 `index.html`、`llms.txt` 與 JSON-LD。`index.html` 內保留可直接讀取的現職、代表重點、工作經歷、代表專案案例與經歷總覽精選項目。
...
經歷項目很多，直接全部展開會降低掃讀效率。新版設計依 `categories` 產生分類區塊，每類預設顯示 `is_featured=true` 的精選項目；若該分類沒有精選項目，則顯示前三筆，並用「展開更多某分類」讓訪客查看完整清單。

### 代表專案採案例式呈現

代表專案使用 `case_study` 欄位呈現「問題、角色、技術、成果」。這讓一般訪客與 HR 不只看到專案名稱，也能快速理解 Yuan 在專案中的實際貢獻與能力範圍。

### 適合聯絡情境

聯絡區明列適合找 Yuan 的情境，包含網路維運、資安訓練平台、AI 自動化、內部工具與技術教育合作。這是為了讓 HR、合作方或社群夥伴更快判斷是否該聯絡。
